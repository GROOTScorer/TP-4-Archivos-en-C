...
void guardar_datos() {
    ...
    for (int i = 0; i < num_registros_cambios; i++) {
        fprintf(archivo, "%s,%s,%d,%d\n", personas[i].nombre, personas[i].apellido, personas[i].dni, personas[i].puntaje);
    }
...
void cargar_cambios() {
    ...
    while (fgets(linea_cambio, sizeof(linea_cambio), archivo)) {
        ...
        token = strtok(NULL, ",");
        personas[num_registros_cambios].dni, token;
        personas[num_registros_cambios].puntaje = atoi(strtok(NULL, ","));
...
void agregar_persona() {
    ...
    scanf("%d", &personas[num_registros_cambios].puntaje);
...
void mostrar_ranking() {
    ...
    for (int i = 0; i < num_registros_cambios - 1; i++) {
        for (int j = 0; j < num_registros_cambios - i - 1; j++) {
            if (personas[j].puntaje < personas[j + 1].puntaje) {
                Persona temp = personas[j];
                personas[j] = personas[j + 1];
                personas[j + 1] = temp;
            }
    ...
    for (int i = 0; i < num_registros_cambios && contador < 10; i++) {
        top_10[contador++] = personas[i];
    }

    printf("Los 10 mejores jugadores:\n");
    for (int i = 0; i < contador; i++) {
        printf("%d. %s - Puntaje: %d\n", i + 1, top_10[i].nombre, top_10[i].puntaje);
    }
}
...
void buscar_por_nombre() {
    ...
    for (int i = 0; i < num_registros_cambios; i++) {
        if (strcmp(personas[i].nombre, nombre) == 0) {
            printf("Nombre: %s\nPuntaje: %d\n", personas[i].nombre, personas[i].puntaje);
            encontrado = 1;
            break;
        }
    }
...
int main() {
    ...
    do {
        ...
        printf("5. Mostrar el top 10 de jugadores\n");
        printf("6. Buscar jugador por nombre\n");
        printf("7. Salir\n");
